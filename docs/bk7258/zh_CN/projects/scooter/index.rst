...
板子使用AP模式::
    ...
    步骤一：通过命令打开板子WIFI；命令为 "test ap <ssid> <key>"；
            ssid和key可以通过传入参数更改；
            默认ssid为"bicycle"，默认key为"12345678"
    步骤二：通过命令查询板子的IP地址，命令为 "ip"
    步骤三：手机连接板子WIFI；
    步骤四：手机端打开两轮车对应的APP，填入板子对应的ip地址，将导航图像投屏到设备端；

板子使用STA模式::

    步骤一：手机打开热点，设置热点名"<ssid>"，wifi密码"<key>"；
    步骤二：通过命令打开板子WIFI连接上手机热点；命令为"test sta <ssid> <key>"；
            ssid和key可以通过传入参数更改；
            默认ssid为"bicycle"，默认key为"12345678"
    步骤三：通过命令查询板子的IP地址，命令为 "ip"
    步骤四：手机端打开两轮车对应的APP，将导航图像投屏到设备端；
...
通过以下两条命令测试::
    ...
    步骤一：输入命令挂载SD卡，命令为"vfs mount fatfs /"
    步骤二：输入命令扫描SD卡中所有文件，命令为"vfs scan /"
    步骤三：输入命令卸载SD卡，命令为"vfs umount /"
...
打开行车记录功能::
    ...
    步骤三：输入命令打开自动录制，命令为"media save_auto 15 10000"
        ...
        15表示每15个文件进行循环；
        文件默认保存在根目录下，文件名默认为auto_0.h264， auto_1.h264，…， auto_14.h264
