...
typedef struct
{
	...
	uint8_t video_transfer : 1;
	uint8_t video_storage : 1;
	uint16_t camera_id;
	uint16_t lcd_id;
...
int av_server_camera_turn_on(void)
{
	bk_err_t ret = BK_FAIL;
	media_camera_device_t device = {0};

	if (db_device_info->camera_id)
	{
		LOGI("%s, already open\n", __func__);
		return BK_FAIL;
	}
	...
	device.fmt = PIXEL_FMT_SMALL_JPEG;
...
int av_server_camera_turn_off(void)
{
	...
	if (db_device_info->camera_id == false)
	{
...
//	media_app_camera_close(NET_CAMERA);

	db_device_info->camera_id = false;
...
{
	if (db_device_info->lcd_id)
	{
		LOGW("%s, already open\n", __func__);
		return BK_FAIL;
	}

	lcd_open_t *lcd_open = (lcd_open_t *)param;
	LOGI("%s, name: %s, rotate: %d\n", __func__, lcd_open->device_name, rotate);

	if (rotate != ROTATE_NONE)
	{
		media_app_lcd_rotate((media_rotate_t)rotate);
	}
...
//	media_app_lcd_pipeline_close();
	media_app_lcd_close(); // total frame decode
