	...
	while (1)
	{
		...
		else
		{
			...
			if (FD_ISSET(db_tcp_service->img_server_fd, &watchfd))
			{
				...
				ret = av_server_display_turn_on(db_tcp_service->lcd, db_tcp_service->rotate);
...
bk_err_t av_server_tcp_service_init(void *param, uint16_t rotate)
{
	int ret;
